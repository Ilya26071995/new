...
// TODO: удаляй неиспользуемые переменные/типизацию
const LABELS = [
    ...
    <div className={s.container}>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className={s.flex}>
        <label className={s.miniFlex} htmlFor='name'>
          Ваше имя
          <input className={s.input} type='text' id='name' {...register("name")} />
          <p>{errors.name?.message}</p>
        </label>
        <label className={s.miniFlex} htmlFor='phone'>
          Телефон
          <input className={s.input} type='text' id='phone' {...register("phone")} />
          <p>{errors.phone?.message}</p>
        </label>
        <label className={s.miniFlex} htmlFor='organization'>
          Название организации
          <input className={s.input} type='text' id='organization' {...register("organization")} />
          <p>{errors.organization?.message}</p>
        </label>
        <label className={s.miniFlex} htmlFor='mail'>
          E-mail
          <input className={s.input} type='text' id='mail' {...register("mail")} />
          <p>{errors.mail?.message}</p>
        </label>
        <div className={s.area}>
          <label htmlFor='text' className={s.label}>
            Текст сообщения
            <textarea id='text' {...register("text")} className={cn(s.input, s.textarea)}></textarea>
        ...
        <div className={s.send}>
          <div className={s.checkbox}>
            ...
            <p className={s.text}>
              ...
              <a href='../files/confidentiality.docx' download className={s.line}>
                Условиями использования и Политикой конфиденциальности.
              </a>
            </p>
          </div>
          <input className={cn(s.btn, !disabled ? s.false : "")} type='submit' value='Отправить' disabled={!disabled} />
        </div>
      </form>
    </div>
  );
